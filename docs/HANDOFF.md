...
```text
d89150a docs: 아이폰 MPO JPEG 지원 기록
f83d0f2 fix: 아이폰 MPO JPEG 압축 지원
```
...
아래 세 가지는 사용자가 필요성을 제기하여 조사와 설계를 시작한 과제입니다.
아직 구현 방식이나 수치를 확정하지 않았으며, 한 번에 하나씩 검토합니다.
...
현재는 위 세 과제의 필요성과 화면 디자인을 먼저 검토합니다. 구현을 시작할 때는
업로드 상한, 속도 측정, 여러 이미지 처리 중 하나만 선택합니다.
...
6. 사용자가 구상한 화면 디자인과 세 개선 과제의 우선순위를 확인합니다.
7. 구현할 경우 한 번에 하나의 작은 과제만 선택합니다.
